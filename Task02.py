...
# Top Products Function: Implement a function using advanced indexing 
# to retrieve transactions of the top 5 products by revenue.
def top_five_prods(transactions: np.ndarray) -> np.ndarray:
    x = np.unique(transactions[2]) # getting product_ids from transactions
    revenues = {}
    # calculating revenue generated by products
    for prod in x:
        indices = transactions[2] == prod
        revenues[prod] = np.sum(transactions[3, indices] * transactions[4, indices])
    # making helper numpy array for sorting
    temp_array = np.array([
                        list(revenues.keys()), 
